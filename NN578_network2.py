...
class LogLikelihood(object):
    ...
    def fn(a, y):
        ...
        ##0 put to stop it from returning as an array with one val.
...
class Network(object):
    ...
    def __init__(self, sizes, cost=CrossEntropyCost, act_hidden=Sigmoid, \
                 act_output=None, regularization=None, dropoutpercent=0.0, dropoutmask = None):
        ...
        for j in range(epochs):#xrange(epochs):
            ...
            for mini_batch in mini_batches:
                if self.dropoutpercent != 0.0 :
                    dropout_mask = np.random.binomial(1, (1-self.dropoutpercent), size = len(mini_batch))/(1-self.dropoutpercent)
                    self.dropoutmask = dropout_mask
                else:
    ...
    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        ...
        for x, y in mini_batch:
            ...
            if self.regularization == 'L1':
                ...
                self.weights = [(1-(eta*(lmbda/n))*(self.sgn(w)))-(eta/len(mini_batch))*nw
        ...
        for b, w in zip(self.biases[:-1], self.weights[:-1]):
                ...
                for b, w in zip(self.biases[1:-1], self.weights[1:-1]):
                    for i in range(len(activations)):
                        if self.dropoutmask != None:
                            activations[i] = activations[i] * self.dropoutmask
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        if self.regularization == 'L1':
            cost += 0.5*(lmbda/len(data))* sum(abs(np.linalg.norm(w)) for w in self.weights)
